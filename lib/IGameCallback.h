...
public:
	...
	void getTilesInRange(std::unordered_set<int3, ShashInt3> &tiles, int3 pos, int radious, boost::optional<PlayerColor> player = boost::optional<PlayerColor>(), int mode = 0, int distanceFormula = 0) const;  //mode 1 - only unrevealed tiles; mode 0 - all, mode -1 -  only revealed
